...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/aslabotHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',
          db.Column('sid', db.Integer, db.ForeignKey('SearchTerm.id'), primary_key=True),
          db.Column('gid', db.Integer, db.ForeignKey('Gif.id'), primary_key=True)
) 
...
user_collection = db.Table('user_collection',
              db.Column('gid', db.Integer, db.ForeignKey('Gif.id'), primary_key=True),
              db.Column('cid', db.Integer, db.ForeignKey('PersonalGifCollection.id'), primary_key=True)
)
...
class User(UserMixin, db.Model):
    ...
    personalGifCollection = db.relationship("PersonalGifCollection")
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return '<Gif %r, %r>' % (self.title, self.embedURL)
...
class PersonalGifCollection(db.Model):
    ...
    gifs = db.relationship("Gif", secondary=user_collection, lazy='dynamic', backref=db.backref('PersonalGifCollection', lazy=True))
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship("Gif", secondary=search_gifs, lazy='dynamic', backref=db.backref('SearchTerm', lazy=True))
    # TODO 364: Define a __repr__ method for this model class that returns the term string
    def __repr__(self):
        return '<SearchTerm %r>' % self.term
...
def get_gifs_from_giphy(search_string):
    ...
    # HINT: You'll want to use 3 parameters in the API request -- api_key, q, and limit. You may need to do a bit of nested data investigation and look
    # for API documentation.
    # HINT 2: test out this function outside your Flask application, in a regular simple Python program, with a bunch of print statements and sample 
    # invocations, to make sure it works!
    baseurl = "https://api.giphy.com/v1/gifs/search"
    extension = "?api_key=" + api_key + "&q=" + search_string + "&limit=5"
    req = requests.get(baseurl+extension)
...
def get_or_create_gif(title, url):
    ...
    g = Gif.query.filter_by(name=title).filter_by(embedURL=url).first()
    if (not g):
        g = Gif(name=title, embedURL=url)    
        db.session.add(g)
        db.session.commit()
    return g
    # Replace with code
...
def get_or_create_search_term(term):
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then
    # append the return value from get_or_create_gif to the search term's associated gifs (remember,
    # many-to-many relationship between search terms and gifs, allowing you to do this!).
    ...
    # HINT: I recommend using print statements as you work through building this function and use it in
    # invocations in view functions to ensure it works as you expect!
    searchTermInstance = SearchTerm.query.filter_by(term=term).first()
    if (not searchTermInstance):
        response = get_gifs_from_giphy(term)
        searchTermInstance = SearchTerm(term=term)
        for gif in response:
            name = gif['title']
            embedURL = gif['embed_url']
            newGif = Gif(name=name, embedURL=embedURL)
            searchTermInstance.gifs.append(newGif)
        db.session.add(searchTermInstance)
        db.session.commit()
        gifs = get_gifs_from_giphy(term)
        for gif in gifs:
            name = gif['title']
            embedURL = gif['embed_url']
            get_or_create_gif(name, embedURL)
    return searchTermInstance
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    s = PersonalGifCollection.query.filter_by(name=name).filter_by(id=current_user).first()
    if (not s):
        s = PersonalGifCollection(id=current_user, name=name)    
        db.session.add(s)
        db.session.commit()
        gifs = get_gifs_from_giphy(name)
        for gif in gifs:
            name = gif['title']
            embedURL = gif['embed_url']
            get_or_create_gif(name, embedURL)
...
def index():
    ...
    # invoke get_or_create_search_term on the form input and redirect to the function corresponding to the path /gifs_searched/<search_term>
    # in order to see the results of the gif search. (Just a couple lines of code!)
    indexForm = GifSearchForm()
    if indexForm.validate_on_submit():
        term = request.indexForm['search']
        searchTerm = get_or_create_search_term(term)
...
def create_collection():
    ...
    gifs = Gif.query.all()
    choices = [(g.id, g.name) for g in gifs]
    ...
    # Use the get_gif_by_id function to create a list of Gif objects.  Then, use the information available to 
    # you at this point in the function (e.g. the list of gif objects, the current_user) to invoke the 
    # get_or_create_collection function, and redirect to the page that shows a list of all your collections.
    ...
    if form.validate_on_submit():
        gif_ids = request.form['gif_picks']
        ids = []
        for id in gif_ids:
            ids.append(get_gif_by_id(id))
        name = request.form['name']
        get_or_create_collection(name, current_user.id, ids)
...
def collections():
    # Replace with code
    # TODO 364: This view function should render the collections.html template so that only the current user's personal gif collection links 
    # will render in that template. Make sure to examine the template so that you send it the correct data!
